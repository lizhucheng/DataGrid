...
cb.model.Model3D.prototype.getData = function (propertyName, onlyCollectDirtyData) {
    ...
    if (onlyCollectDirtyData && propertyName == "value") {
        var datas = [];
        var rows = this._rowsInView; ////this._data.Cache;???//多页数据怎么处理
        ...
        if (propertyName == "value") {
            var rows = this._rowsInView.clone(); //this._data.Cache;???//多页数据怎么处理
            if (this._rowsInView.length == 0) rows.length = 0;
...
$.extend(cb.model.Model3D.prototype,{
	...
	get:function (rowIndex, cellName, propertyName) {
		...
		if (!propertyName || propertyName.toLowerCase() === "value") {
			...
			var row = rowIndex >= 0 ? this._rowsInView[rowIndex] : null;
			if (!row || !cellName)
			...
			if (rowIndex < 0) {
				...
				if (!cellName)
					return this._rowsInView[rowIndex][propertyName];
				var cell = this._rowsInView[rowIndex][cellName];
	...
	set:function (rowIndex, cellName, propertyName, value) {
		...
		if (!propertyName) {
			if (rowIndex < 0 || !cellName)
				return;
			var row = this._rowsInView[rowIndex]; // this.get(rowIndex);
			var cell = row[cellName];
			...
			//设置行状态
				...
				var oldValue = this._rowsInView[rowIndex][propertyName];
				if (oldValue === value)
					return;
				...
				if (!value && (propertyName == "readOnly" || propertyName == "disabled")) {
					//如果值==false,
					delete this._rowsInView[rowIndex][propertyName];
				}
				else {
					this._rowsInView[rowIndex][propertyName] = value;
				}
			...
			//设置单元格状态
				...
				if (cb.isEmpty(value)) {
					...
					//this._rowsInView[rowIndex][cellName] = isObject ? cell.Value : cell; //不止一个属性
				}
				else if (!value && (propertyName == "readOnly" || propertyName == "disabled")) {
					//如果值==false,
					//this._rowsInView[rowIndex][cellName] = isObject ? cell.Value : cell;
					if (isObject) {
						delete cell[propertyName];
						var hasProperty = false;
						cb.eachIn(cell, function (attr) { if (attr != "Value" || attr != "value") { hasProperty = true; return; } });
						if (!hasProperty)
							this._rowsInView[rowIndex][cellName] = cell.Value;
					}
				}
				else {
					if (!isObject)
						cell = this._rowsInView[rowIndex][cellName] = { value: cell };
	...
	setMergeState:function(merge){
		if(this.set('mergeState',!!merge)!==false){//属性变化时才通知视图更新，把和并信息的计算放到绑定器中处理
			...
			var args = new cb.model.PropertyChangeArgs(this._name, "mergeInfo",{rows:this._rowsInView,mergeCells:mergeCells});
			this.PropertyChange(args);	
		}
	},
	...
	setSortFields:function(sortFields,noReflesh){//noReflesh仅内部使用
		...
		if(this.set('sortFields',sortFields)!==false && !noReflesh && sortFields.length){//如果指定不刷新这不重排序
			this._rowsInView = this._sort(this._rowsInView);
			//显示的数据集（包括排序信息）改变后，刷新界面显示
			this.PropertyChange(new cb.model.PropertyChangeArgs(this._name, "displayRows", this._rowsInView));
			this._after("sort");
		}
	...
	*/
	...
	_setPageRows:function(rows){
		this._rowsInView=this._sort(this._filter(rows));
		//刷新视图
		this.PropertyChange(new cb.model.PropertyChangeArgs(this._name, "displayRows", this._rowsInView));
	},
	...
	commitRows:function (rows) {
		...
		cb.each(rows, function (row) {
			var rowIndex = (typeof row == "number") ? row : this._rowsInView.indexOf(row);
	...
	getRow:function (rowIndex) {
		return this._rowsInView[rowIndex];
	},
	getRowIndex:function (row) {
		return this._rowsInView.indexOf(row);
	},
	//焦点管理
	...
	select:function (rows) {
		...
		cb.each(rows, function (index) {
			if (index == this._rowsInView.length) return;
			this._rowsInView[index].isSelected = true;
		}, this);
	...
	unselect:function (rows) {
		...
		cb.each(rows, function (index) { this._rowsInView[index].isSelected = false; }, this);
	...
	selectAll:function () {
		...
		cb.each(this._rowsInView, function (row) { row.isSelected = true; }, this);
	...
	unselectAll:function () {
		...
		cb.each(this._rowsInView, function (row) { row.isSelected = false; }, this);
	...
	add:function (rows, isRemoveAll) {
		...
		rows = cb.isArray(rows) ? rows : [rows];
		for (var i = 0; i < rows.length; i++) {
			this._rowsInView.push(rows[i]); //rows可以为多行,[]
			if (!rows[i].state)
				rows[i].state == cb.model.DataState.Add; //新增行
			this._$setId(rows[i]);
		}
		if (!this._focusedRow) {
			this.setFocusedRow(this._rowsInView[0]);
		}
	...
	insert:function (rowIndex, row) {
		...
		this._rowsInView.insert(rowIndex, row);

		this._$setId(row);
		this._processRow(row);

		if (willSetFocusedRow == true) {
			this.setFocusedRow(this._rowsInView[rowIndex]);
		}
	...
	remove:function (rows) {
		...
		if (cb.isArray(rows)) {
			for (var j = 0; j < rows.length; j++) {
				var index = (typeof rows[j] == "number") ? rows[j] : this._rowsInView.indexOf(rows[j]);
				deleteRows.push(index);
			}
			deleteRows.sort(function (a, b) { return a < b ? 1 : -1; });
			cb.each(deleteRows, function (k) {
				this._backupDeleteRows(this._rowsInView[k]);
				this._rowsInView.remove(k);
			}, this);
		}
		else {
			var index2 = (typeof rows == "number") ? rows : this._rowsInView.indexOf(rows);
			deleteRows.push(index2);
			this._backupDeleteRows(this._rowsInView[index2]);
			this._rowsInView.remove(index2);
		}
