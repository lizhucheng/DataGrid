...
function NumberFormatter(value,dataContext){//scale:小数位数，precision：精度，两者冲突时以小数位为准
	var scale=dataContext.scale,
		precision=dataContext.precision;
	value=Math.floor((scale+1)*100);//整数化到小数位后面一位
	value=(value/10+value%10/5)/(scale*100);//保留scale位小数，且scale后面位四舍五入
	return value;
}
//
...
function ReferFormatter(value,dataContext){//length:20,refKey:"id",refCode:"code",refName:"name",ctrlType:"Refer",refId:"Computation",refShowMode:"Code"
	var refKey=this[FIELDNAME_PROP];
	var showValue=dataContext[refKey+'_'+dataContext.refShowMode]
